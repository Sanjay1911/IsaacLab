...
class MinimalSceneCfg(InteractiveSceneCfg):
    ...
    # vessel_10 = AssetBaseCfg(
    #     prim_path="{ENV_REGEX_NS}/VesselPost10",
    #     spawn=sim_utils.MeshFileCfg(
    #         file_path="/home/sanjay/thesis_replications/forked/IsaacLab/custom/objs/deformed_env0_entry0_step10.obj"
    #     ),
    #     init_state=AssetBaseCfg.InitialStateCfg(pos=(0.0, 0.0, 0.20), rot=(0.70710, 0.70710, 0.0, 0.0)),
    # )
    ...
    vessel = AssetBaseCfg(
        ...
        spawn=sim_utils.MeshFileCfg(
            file_path="/home/sanjay/thesis_replications/forked/IsaacLab/custom/objs/deformed_env0_entry0_step10_new_noisy.obj"
        ),
        init_state=AssetBaseCfg.InitialStateCfg(pos=(0.0, 0.0, 0.20), rot=(0.70710, 0.70710, 0.0, 0.0)),
    )

    # vessel_ref = AssetBaseCfg(
    #     prim_path="{ENV_REGEX_NS}/VesselRef",
    ...
    raycast_camera = RayCasterCameraCfg(
        ...
        mesh_prim_paths=["{ENV_REGEX_NS}/Tumor", "{ENV_REGEX_NS}/VesselPre"],
        update_period=0.1,
...
# Sample 100 random tumor poses
    if MODE == "test":
        ...
        # Load the vessel mesh from already-loaded USD stage (deformed)
        vessel_meshes = get_trimesh_mesh("vessel")  # Make sure this is now deformed
        vessel_points, _ = sample_even_fit_mesh(vessel_meshes[0][0], n_spheres=20000, sphere_radius=0.005)

        # Load original dataset (pre-deformation)
        data = load_pickle("/home/sanjay/thesis_replications/forked/IsaacLab/tumor_dataset_100_2205.pkl")    

        for i in range(min(1,len(data))):
            print(f"[INFO] Re-scoring data for env {i}")

            try:
                env_data = data[i]
                top_paths = env_data["top_entry_points"]
                tumor_centroid = env_data["tumor_centroid"]

                # Recompute scores on the deformed mesh
                for path in top_paths:
                    entry_point = path["entry_point"]
                    score_post, ratio_post = collision_scoring(entry_point, tumor_centroid, vessel_points)
                    path["score_post"] = score_post
                    path["ratio_post"] = ratio_post

                dataset[i] = {
                    "tumor_position": env_data["tumor_position"],
                    "tumor_quat": env_data["tumor_quat"],
                    "tumor_centroid": tumor_centroid,
                    "top_entry_points": top_paths  # Now contains score_post and ratio_post
                }

            except Exception as e:
                print(f"[ERROR] Failed to process env {i}: {e}")
                continue

        # Save new dataset with post-deformation scoring
        dump_pickle("/home/sanjay/thesis_replications/forked/IsaacLab/custom/path_comparison/tumor_dataset_post_1111.pkl", dataset)
        print("✅ Finished saving post-deformation scores.")

    elif MODE == "dataset":
        tumor_positions, tumor_quats = sample_random_tumor_pose(10)
        dataset = {}
        ...
        vessel_points, _ = sample_even_fit_mesh(vessel_meshes[0][0], n_spheres=20000, sphere_radius=0.005)
        print(f"[INFO] Sampled {len(vessel_points)} vessel points.")
        ...
        for i in range(1):
            ...
            if len(top_paths) == 0:
                ...
                continue
            #print top path and score
            print(f"[INFO] Tumor {i} Top Path: {top_paths[0]['entry_point']}, Score: {top_paths[0]['score']}, Ratio: {top_paths[0]['ratio']}")
            start_pose = []
            ...
            if i == 10:
                print("[INFO] Reached 10 tumors, breaking loop.")
                break

        # Save dataset
        dump_pickle("/home/sanjay/thesis_replications/forked/IsaacLab/custom/path_comparison/tumor_dataset_1_0606.pkl", dataset)
        print("✅ Finished saving dataset with 10 tumor entries.")
