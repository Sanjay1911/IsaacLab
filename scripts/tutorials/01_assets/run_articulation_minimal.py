...
def main():
    ...
    scene_cfg = MinimalSceneCfg(num_envs=4, env_spacing=2.0)
    scene = InteractiveScene(scene_cfg)
    sim.reset()

    robot = scene["robot"]
    skull = scene["skull"]
    skull_mesh_prim = sim_utils.find_first_matching_prim(prim_path_regex="/World/envs/env_.*/Skull")
    print("[INFO] Mesh Prim: ", skull_mesh_prim, skull_mesh_prim.GetTypeName())
    print("[INFO] Mesh Prim Children: ", skull_mesh_prim.GetAllChildren())
    mesh_prim = sim_utils.get_first_matching_child_prim(
        str(skull_mesh_prim.GetPath()), lambda prim: prim.GetTypeName() == "Mesh"
    )

    if mesh_prim is None or not mesh_prim.IsValid():
        raise RuntimeError(f"Invalid mesh prim at path: {skull_mesh_prim.GetPath()}")
    ...
    try:
        ...
        print("[ERROR] Failed to convert to mesh prim due to : ", e)
        pass

    # print("[INFO] Table: ", table.get_local_poses(), table.get_world_poses())
    # print("[INFO]: SKULL: ", skull.get_local_poses(), skull.get_world_poses())
